...
const OneToOneChat = ({ navigation, route }) => {
    ...
    // console.log(initialMessages, 'initial message ');
    ...
    //         flatListRef.current.scrollToEnd({ animated: false }); // no animation on first load
    //     }
    // }, [messages]);
    ...
    useEffect(() => {
        ...
        on('typingResponse', (data) => {
            // console.log('response from typing', data);

            if (data?.response?.message !== null && data?.response?.senderId === (item?.participantId._id || id)) {
                setIsTyping(true);
                clearInterval(typingIndicatorTimeout);
            } else {
                setIsTyping(false);
    ...
    useEffect(() => {
        on('messageResponse', (response) => {
            ...
            if (response?.result && response?.result?.OneToOneId === roomId) {
                setMessages((prevMessages) => {
            ...
            {ispremiumuser ? (
                <FlatList
                    ...
                    renderItem={({ item }) => {
                        ...
                        // console.log('ispicrequest', isPicRequest);
