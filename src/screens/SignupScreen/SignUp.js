...
const SignUp = ({ navigation }) => {
    ...
    const [tokenforlogin, setToken] = useState('')
    ...
    const onMessage = event => {
        ...
        if (event && event.nativeEvent.data) {
            if (['cancel', 'error', 'expired'].includes(event.nativeEvent.data)) {
                ...
                // //('Verified code from Google', event.nativeEvent.data);
                setTimeout(() => {
                    captchaFormRef.current.hide();
    ...
    //             }
    //             if (data?.preferences?.gender) {
    //                 setSelectInterest(data?.preferences?.gender)
    //             }
    ...
    const createSignUp = async () => {
        ...
        if (
            ...
            const body = {
                email: email,
                gender: selectedgender === 'Non-binary' ? 'Other' : selectedgender,
    ...
    const verifyOtp = async (body) => {
        try {
            ...
            if (resp?.data?.data?.user) {
                ...
                setToken(token)
                //('token forrrrrrr>>>>>>>>>>', token);

                if (user?.email && user?.profileCompleted === true) {
                    navigation.navigate('Login');
                    Toast.show('User Already Registered, Please Sign In', Toast.SHORT);
                } else {
                    const step = user?.step || 0;
                    if (step > 6) {
                        navigation.navigate('ProfileSignUp', { step, email: user?.email, logintoken: token });
                        Toast.show(resp?.message || 'Profile setup needed.', Toast.SHORT);
                    } else {
                        navigation.navigate('SignUp', { step })
                        Toast.show(resp?.message || 'Sign up process started.', Toast.SHORT)
                    }
                }
                return resp.data;
            } else {
                throw new Error('Invalid OTP or user details missing.');
            }
        } catch (error) {
            ...
            return null; // Return null to avoid proceeding if OTP verification fails
        }
    };
    ...
    const createPassword = async () => {
        ...
        //('Create password request body:', body);
        try {
    ...
    const getSecurityQuestion = async () => {
        ...
        // //('Body of security question:', body);
        ...
        try {
            ...
            const body = {
                ...
                preferences: {
                    ...
                    gender: selectinterest,
                    distance: 700,
                    aboutPartnerDescription: ""
                },
                dateOfBirth: moment(selectedDate).format('YYYY-MM-DD')
            };
            //('body of google signup', body);
            try {
                const response = await axios.post('auth/sign-in-google', body);
                console.log('responsde from google mai?????? signuppp1 ...');
                await AsyncStorage.setItem('verifcationToken', response?.data?.data?.token);
                //('response from the google signUp', JSON.stringify(response?.data));

                // Store the userId and proceed with login if profile is completed
                const userId = response.data?.data?.user?._id;
                setUserId(userId);
                if (response?.data?.data?.user?.step === 1) {
    ...
    const handleDateConfirm = async (date) => {
        ...
        if (userAge >= 18 && userAge <= 100) {
            ...
            Toast.show('You must be 18-100 years old to use this service.', Toast.SHORT);
            setDatePickerOpen(false);
        }
    };
    ...
    useEffect(() => {
        if (currentStep === 4 && intervalRef.current === null) {
            setTimer(60); // reset timer when reaching step 4
    ...
    // useEffect(() => {
    //     let interval;
    //     // Start the timer only when the user reaches Step 4
    ...
    //     // Clean up the interval if the timer reaches 0 or user navigates away
    ...
    const handleOtpChange = (value, index) => {
        ...
        if (enteredOtp.length === otp.length) {
            setOtpValid(true); // Enable submit button or proceed
        } else {
            setOtpValid(false); // Disable submit button if OTP is invalid
        }
    };

    const getFilteredQuestions = (excludedQuestions) => {
        return questionsList.filter(q => !excludedQuestions.includes(q.label));
    };



    const renderDropdownWithInput = (
        label,
        selectedValue,
        setSelectedValue,
        answerValue,
        ...
        <View style={styles.dropdownContainer}>
            ...
            <TextInput
                style={[
                    styles.textInput,
                    selectedValue === '' && { backgroundColor: '#f0f0f0' }  // Light gray background when disabled
                ]}
    ...
    const handleContinue = async () => {
        Keyboard.dismiss();
        setLoading(true);  // Show loader when continue is pressed
        try {
            if (currentStep === 6) {
                ...
                try {
                    ...
                    navigation.navigate('ProfileSignUp', { userId: userId, password: password, email: email, fcmtoken: fcmtoken, logintoken: tokenforlogin, idToken: idToken }); // Proceed to the ProfileSignUp screen
                } catch (error) {
            ...
            if (currentStep === 2) {
                ...
                if (selectedDateWithoutTime.getTime() === todayWithoutTime.getTime()) {
                    Toast.show('You cannot select today\'s date. Please select a date in the past.', Toast.SHORT);
                    setLoading(false);  // Hide loader after the operation
                    return;
                }
            }
            ...
            setLoading(false);  // Hide the loader after the process is done
        }
    };
    ...
    const handleBack = () => {
        if (currentStep === 6) {
            setCurrentStep(3); // Redirect to Step 3 if on Step 6
        } else if (currentStep > 1) {
            setCurrentStep(currentStep - 1); // For other steps, just go to the previous step
        }
    };
                ...
                return (
                    <ScrollView>
                        <View>
                            ...
                            {['Male', 'Female', 'Both', 'Non-binary'].map((option) => (
                                <TouchableOpacity
                                    key={option}
                                    style={[
                                        styles.optionContainer,
                                        selectinterest[0] === option && styles.selectedBodyTypeButton
                                    ]}
                                    onPress={() => handleInteretSelect(option)}>
                                    <Text style={[
                                        styles.optionText,
                                        selectinterest[0] === option && styles.selectedBodyTypeText
                                    ]}>
                                        {option}
                                    </Text>
                                </TouchableOpacity>
            ...
            case 4:
                return (
                    <View >
                        ...
                        <Text style={[styles.txt14, { marginTop: 10 }]}>
            ...
            case 5:
                return (
                    <View>
                        ...
                        <Text style={[styles.txt14, { marginTop: 10 }]}>To ensure your account's security, please choose a {'\n'} new password.</Text>
                        ...
                        <View style={styles.inputContainer}>
                            <TextInput
                                ...
                                onChangeText={(text) => {
                                    ...
                                    setIsPasswordMatched(text !== '' && text === confirmPassword);
                        ...
                        <View style={styles.inputContainer}>
                            <TextInput
                                ...
                                onChangeText={(text) => {
                                    ...
                                    setErrorMessage('');  // Reset error message when user starts typing
                                    const matched = password !== '' && password === text;
                                    setIsPasswordMatched(matched);
                                    if (matched) {
                                        Keyboard.dismiss(); // 👈 Hides keyboard when passwords match
                                    }
                        ...
                        {isPasswordMatched && (
                            <LottieView
                                source={require('../../assets/confirm.json')}  // Replace with your Lottie file
                                autoPlay
                                loop={false}  // Set to false to play once
                                style={{ width: 50, height: 50, marginTop: 10, alignSelf: 'center' }}  // Customize style as needed
                            />
                        )}
                    </View>
                );
                ...
                return (
                    <ScrollView style={{}}>
                        <View>
                            ...
                            <Text style={[styles.txt14, { marginTop: 10 }]}>To help secure your account, please select three {'\n'} security questions and provide answers.This will be{'\n'}used to verify your identity in case you need to reset {'\n'} your password later.</Text>

                            {renderDropdownWithInput(
                                'Select Question 1',
                                question1,
                                setQuestion1,
                                answer1,
                                setAnswer1,
                                getFilteredQuestions([question2, question3])
                            )}
                            ...
                            {renderDropdownWithInput(
                                ...
                                answer2,
                                setAnswer2,
                                getFilteredQuestions([question1, question3])
                            )}
                            ...
                            {renderDropdownWithInput(
                                ...
                                answer3,
                                setAnswer3,
                                getFilteredQuestions([question1, question2])
                            )}
        ...
        <View style={styles.container}>
            ...
            <View style={styles.footer}>
                ...
                <View style={styles.footerButtonsContainer}>
                    ...
                    >
                        {loading ? (
                            <ActivityIndicator size="small" color="#fff" /> // Loader displayed here
                        ) : (
