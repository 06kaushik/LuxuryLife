...
const NewestScreen = ({ navigation, index }) => {
    ...
    useEffect(() => {
        if (isFocused && userdetails && index === 1) {
            getUserFilteredData();
        }
    ...
    const getUserFilteredData = async (page) => {
        ...
        // //('body of newest', JSON.stringify(body))
        setIsLoading(true);
        try {
            const resp = await axios.post('home/search', body, { headers });
            //('response from the search API in NEWEST', resp?.data?.data?.length);
            setUserData(
