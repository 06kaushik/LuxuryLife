...
import { Text, View, TouchableOpacity, Image, StyleSheet, FlatList, ScrollView, TextInput, Dimensions, ActivityIndicator, Keyboard } from "react-native";
...
const FileredUsers = ({ navigation }) => {
    ...
    const handleSuggestionPress = (item) => {
        //('formatted address', JSON.stringify(item));
    ...
    const getUserSearch = async () => {
        ...
        try {
            ...
            // //('Saved filters:', JSON.stringify(validFilters));
        } catch (error) {
            //('Error from get search', error.response?.data?.message);
        }
    };

    const updateUserSearch = async (userId) => {
        //('iddd to updatee', userId);
        ...
        try {
            ...
            //('response of update user search', resp.data);
            setIsModalVisible(false);
        } catch (error) {
            //('error from the user search', error.response.data.message);
        }
    }
    ...
    const handleDeleteFilter = async (id) => {
        ...
        try {
            ...
            //('response from the delete api filter', resp.data);
            getUserSearch()
        } catch (error) {
            //('error from the delete filter api', error);
        }
    }
    ...
    const saveUserSearch = async () => {
        ...
        //('body of saved filter', body);

        try {
            const resp = await axios.post('home/save-user-filter', body, { headers })
            //('resonse from save filter', resp.data);
            getUserSearch();
            setIsModalVisible(false);
        } catch (error) {
            //('error from the save filters>>>>>>>', error.response.data.message);
        }
    }
    ...
    const getSearch = async () => {
        ...
        // //('body of preferece for searchhhh', body);

        try {
            const resp = await axios.post('home/search', body, { headers })
            // //('response from the get search api', resp?.data);
            ...
            //('error from the get search api', error.response.data.message);
        ...
        <View style={styles.main}>
            ...
            <ScrollView style={styles.content} keyboardShouldPersistTaps="handled" >
                ...
                <TouchableOpacity onPress={toggleSeekingVisibility} style={styles.sectionHeader}>
                    <Text style={styles.sectionTitle}>SHOW MEMBERS WITH LUXE INTERESTS IN</Text>
                    <Image
