...
const IncomingAudioCallHandler = ({ }) => {
    ...
    const acceptCall = () => {
        ...
        setIncomingCall(null);
        navigation.navigate('IncomingAudioCall', {
            selectedUser: incomingCall.userId,
            videoRoomId: incomingCall.roomId,
            userId: userId,
            userName: incomingCall?.userDetails?.userName,
            profilePicture: incomingCall?.userDetails?.profilePicture,
            logId: incomingCall?.logId
        });
    };
    ...
    useEffect(() => {
        const handleUserConnected = (data) => {
            if (data === userId) {
                //('🧹 Clearing UI from IncomingCallHandler because user joined on another device');
                stopRingtone();
                setIncomingCall(null);
            }
        };
            ...
            <View style={styles.innerRow}>
                ...
                <View style={styles.buttonContainer}>
                    <TouchableOpacity style={styles.accept} onPress={acceptCall}>
                        <LottieView
                            source={require('../assets/audiocall.json')}
                            autoPlay
                            loop
...
export default IncomingAudioCallHandler;
